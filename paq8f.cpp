...
public:
  ...
  int level;
  int MEM; // size of working space

  GlobalContext(Buf & _buf, int _level = DEFAULT_OPTION) 
    : buf(_buf) {
    level = _level;
    MEM  = (0x10000<<level);
    int y=0; 
...
class Mixer {
  ...
  GlobalContext& gc; 
public:
  Mixer(GlobalContext& gc, int n, int m, int s=1, int w=0);

  // Adjust weights to minimize coding cost of last prediction
  void update() {
    for (int i=0; i<ncxt; ++i) {
      int err=((gc.y<<12)-pr[i])*7;
...
Mixer::Mixer(GlobalContext& _gc, int n, int m, int s, int w):
  gc(_gc), N((n+7)&-8), M(m), S(s), tx(N), wx(N*M),
  ...
  for (int i=0; i<N*M; ++i)
    wx[i]=w;
  if (S>1) mp=new Mixer(gc, S, 1, 1, 0x7fff);
}
...
APM::APM(GlobalContext& _gc, int n): gc(_gc), index(0), N(n), t(n*33) {
  for (int i=0; i<N; ++i)
    for (int j=0; j<33; ++j)
...
public:
  StateMap();
  StateMap(GlobalContext & gc);
  int p(int cx) {
    assert(cx>=0 && cx<t.size());
    t[cxt]+=(gc->y<<16)-t[cxt]+128 >> 8;
    return t[cxt=cx] >> 4;
  }
  void set_gc(GlobalContext& _gc) {
    gc = & _gc;
  }
  static StateMap* make_array(GlobalContext& _gc, int len) {
    StateMap* array = new StateMap[len];
    for (int i=0; i<len; i++)
      array[i].set_gc(_gc);
    return array;
  }
...
StateMap::StateMap(GlobalContext& _gc): gc(&_gc), cxt(0), t(256) {
  for (int i=0; i<256; ++i) {
    int n0=nex(i,2);
    int n1=nex(i,3);
    if (n0==0) n1*=64;
...
class RunContextMap {
  ...
  U8* cp;
public:
  RunContextMap(GlobalContext& _gc, int m): gc(_gc), t(m/4) {cp=t[0]+1;}
  void set(U32 cx) {  // update count
    if (cp[0]==0 || cp[1]!=gc.buf(1)) cp[0]=1, cp[1]=gc.buf(1);
    else if (cp[0]<255) ++cp[0];
    cp=t[cx]+1;
  }
  int p() {  // predict next bit
    if (cp[1]+256>>8-gc.bpos==gc.c0)
      return ((cp[1]>>7-gc.bpos&1)*2-1)*ilog(cp[0]+1)*8;
    else
      return 0;
  }
...
class SmallStationaryContextMap {
  ...
  U16 *cp;
public:
  SmallStationaryContextMap(GlobalContext& _gc, int m): gc(_gc), t(m/2), cxt(0) {
...
public:
  ContextMap(GlobalContext& _gc, int m, int c=1);  // m = memory in bytes, a power of 2, C = c
  void set(U32 cx);   // set next whole byte context
  int mix(Mixer& m) {return mix1(m, gc.c0, gc.bpos, gc.buf(1), gc.y);}
};
...
ContextMap::ContextMap(GlobalContext& _gc, int m, int c): 
  gc(_gc), C(c), t(m>>6), cp(c), cp0(c),
  cxt(c), runp(c), cn(0) {
  assert(m>=64 && (m&m-1)==0);  // power of 2?
  assert(sizeof(E)==64);
  sm=StateMap::make_array(gc, C);
  for (int i=0; i<C; ++i) {
...
int ContextMap::mix1(Mixer& m, int cc, int bp, int c1, int y1) {
  ...
  for (int i=0; i<cn; ++i) {
    ...
    if (gc.bpos>1 && runp[i][0]==0)
      cp[i]=0;
    else if (gc.bpos==1||gc.bpos==3||gc.bpos==6)
      cp[i]=cp0[i]+1+(cc&1);
    else if (gc.bpos==4||gc.bpos==7)
      cp[i]=cp0[i]+3+(cc&3);
    else {
...
public:
  ...
  ModelMatcher(GlobalContext &_gc) : gc(_gc), MAXLEN(2047), t(gc.MEM) {
    //MAXLEN=2047;  // longest allowed match + 1
    h=0;  // hash of last 7 bytes
    ptr=0;  // points to next byte of match if any
    len=0;  // length of match, or 0 if no match
    result=0;
  }
...
// matchModel() finds the longest matching context and returns its length
  ...
  int match(Mixer& m) {
    ...
    if (!gc.bpos) {
      h=h*997*8+gc.buf(1)+1&t.size()-1;  // update context hash
      if (len) ++len, ++ptr;
      else {  // find match
	ptr=t[h];
	if (ptr && pos-ptr<gc.buf.size())
	  while (gc.buf(len+1)==gc.buf[ptr-len-1] && len<MAXLEN) ++len;
      }
    ...
    int sgn;
    if (len && gc.buf(1)==gc.buf[ptr-1] && gc.c0==gc.buf[ptr]+256>>8-gc.bpos) {
      if (gc.buf[ptr]>>7-gc.bpos&1) sgn=1;
      else sgn=-1;
    }
    else sgn=len=0;
...
public:
  ...
  GlobalContext& gc;
  U32 r0, r1, r2, r3;  // last 5 rows, bit 8 is over current pixel
  Array<U8> t;  // model: cxt -> state
  int cxt[PicModel_N];  // contexts
  StateMap sm[PicModel_N];

  PicModel(GlobalContext& _gc) : 
    gc(_gc), t(0x10200) {
    for (int i=0; i<PicModel_N; i++)
      sm[i].set_gc(_gc);
  }
  ...
  void run(Mixer& m) {
    ...
    // update the model
    for (int i=0; i<PicModel_N; ++i)
      t[cxt[i]]=nex(t[cxt[i]],gc.y);

    // update the contexts (pixels surrounding the predicted one)
    r0+=r0+gc.y;
    r1+=r1+((gc.buf(215)>>(7-gc.bpos))&1);
    r2+=r2+((gc.buf(431)>>(7-gc.bpos))&1);
    r3+=r3+((gc.buf(647)>>(7-gc.bpos))&1);
...
public:
  GlobalContext& gc;
  U32 word0, word1, word2, word3, word4;  // hashes
  U32 text0;  // hash stream of letters
  ContextMap cm;
  Array<int> wpos;  // last position of word
  int nl1, nl;  // previous, current newline position

  WordModel(GlobalContext& _gc) : gc(_gc), cm(_gc, gc.MEM*32, 14), wpos(gc.MEM) {
    word0=0, word1=0, word2=0, word3=0, word4=0;  // hashes
    text0=0;  // hash stream of letters
    nl1=-3, nl=-2;  // previous, current newline position
  }
  ...
  void run(Mixer& m) {
	...
	word0=0;
      ...
      int col=min(255, pos-nl), above=gc.buf[nl1+col]; // text column context
      U32 h=word0*271+gc.buf(1);
      cm.set(h);
      cm.set(word0);
      cm.set(h+word1);
      ...
      // Text column models
      cm.set(col<<8|above);
      cm.set(col<<8|gc.buf(1));
      cm.set(gc.buf(1)<<8|above);
      cm.set(col);
    }
    cm.mix(m);
  }
...
public:
  GlobalContext &gc;
  Array<int> cpos1, cpos2, cpos3, cpos4; //buf(1)->last 3 pos
  Array<int> wpos1; // buf(1..2) -> last position
  int rlen, rlen1, rlen2;  // run length and 2 candidates
  int rcount1, rcount2;  // candidate counts
  ContextMap cm;

  RecordModel(GlobalContext& _gc) : 
    gc(_gc),  
    cpos1(256), cpos2(256), cpos3(256), cpos4(256), 
    wpos1(0x10000), 
    rlen(2), rlen1(3), rlen2(4), 
    rcount1(0), rcount2(0), cm(gc, gc.MEM*4, 7) {
  }
  ...
  void run(Mixer & m) {
    ...
    if (!gc.bpos) {
	  ...
	  && (r>15 || (c==gc.buf(r*5+1)) && c==gc.buf(r*6+1))) {
	if (r==rlen1) ++rcount1;
      ...
      assert(rlen>0);
      cm.set(gc.buf(1)<<8|min(255, pos-cpos1[gc.buf(1)]));
      cm.set(gc.buf(1)<<17|gc.buf(2)<<9|llog(pos-wpos1[w])>>2);
      int col=pos%rlen;
      cm.set(gc.buf(1)<<8|gc.buf(rlen));
      cm.set(rlen|gc.buf(rlen)<<10|gc.buf(rlen*2)<<18);
      cm.set(rlen|gc.buf(rlen)<<10|col<<18);
      cm.set(rlen|gc.buf(1)<<10|col<<18);
      cm.set(col|rlen<<12);
...
public:
  ...
  SparseModel(GlobalContext& _gc) :
    gc(_gc), cm(gc, gc.MEM*4, 8), scm(gc, gc.MEM, 8) {
  }

  void run(Mixer& m) {
    if (gc.bpos==0) {
      cm.set(gc.c4&0x00ff00ff);
      cm.set(gc.c4&0xff0000ff);
      cm.set(gc.buf(1)|gc.buf(5)<<8);
      cm.set(gc.buf(1)|gc.buf(6)<<8);
      cm.set(gc.c4&0x00ffff00);
      cm.set(gc.c4&0xff00ff00);
      cm.set(gc.buf(3)|gc.buf(6)<<8);
      cm.set(gc.buf(4)|gc.buf(8)<<8);
      for (int i=0; i<8; ++i)
	scm.set(gc.buf(i+1));
    }
    cm.mix(m);
...
public:
  ...
  IndirectModel(GlobalContext &_gc) : gc(_gc), cm(gc, gc.MEM*4, 5), t1(256), t2(0x10000) {}
  ...
  void run(Mixer & m) {
    ...
    if (!gc.bpos) {
      U32& r1=t1[gc.buf(2)];
      r1=r1<<8|gc.buf(1);
      U16& r2=t2[gc.buf(3)<<8|gc.buf(2)];
      r2=r2<<8|gc.buf(1);
      U32 t=gc.buf(1)|t1[gc.buf(1)]<<8;
      cm.set(t&0xffff);
      cm.set(t&0xffffff);
      cm.set(t);
      t=gc.buf(2)<<8|gc.buf(1)|t2[gc.buf(2)<<8|gc.buf(1)]<<16;
      cm.set(t&0xffffff);
      cm.set(t);
    }
    cm.mix(m);
...
public:
  ...
  Filetype filetype;
  int size;  // bytes remaining in block
  ...
  ContextModel2(GlobalContext& _gc) :
    gc(_gc), 
    cm(gc, gc.MEM*32, 9), 
    rcm7(gc, gc.MEM), rcm9(gc, gc.MEM), rcm10(gc, gc.MEM), 
    m(gc, 512, 1040, 4, 128), 
    filetype(DEFAULT), 
    size(0),
    modelmatcher(_gc),
  ...
  int run() {
    ...
    if (gc.bpos==0) {
      --size;
      if (size==-1) filetype=(Filetype)gc.buf(1);
      if (size==-5) {
	size=gc.buf(4)<<24|gc.buf(3)<<16|gc.buf(2)<<8|gc.buf(1);
    ...
    int isjpeg=0;//jpegModel(m);  // 1 if JPEG is detected, else 0
    int ismatch=modelmatcher.match(m);  // Length of longest matching context
    int isbmp=0;//bmpModel(m);  // Image width (bytes) if BMP or TIFF detected, or 0
    ...
    if (ismatch>400) {   // Model long matches directly
      ...
      */
      ...
      m.set(2, 8);
      m.set(col, 24);
      m.set(gc.buf(isbmp)+gc.buf(3)>>4, 32);
      m.set(c0, 256);
      return m.p();
      */
    }
    ...
    m.set(gc.buf(1)+8, 264);
    m.set(gc.c0, 256);
    m.set(order+8*(gc.c4>>5&7)+64*(gc.buf(1)==gc.buf(2))+128*(filetype==EXE), 256);
    m.set(gc.buf(2), 256);
    int pr=m.p();
    return pr;
  }
...
class Predictor {
  ...
  ContextModel2 cm;
public:
  Predictor(GlobalContext& _gc);
  int p() const {assert(pr>=0 && pr<4096); return pr;}
  void update();
};

Predictor::Predictor(GlobalContext& _gc)
  : gc(_gc), pr(2048), 
    a1(gc, 256), a2(gc, 0x10000), a3(gc, 0x10000), a4(gc, 0x10000),
    cm(gc) 
{}
...
void Predictor::update() {
  ...
  pr=cm.run();
  pr=a1.p(pr, gc.c0)*3+pr>>2;
  int pr2=a2.p(pr, gc.c0+256*gc.buf(1));
  int pr3=a3.p(pr, gc.c0^hash(gc.buf(1), gc.buf(2))&0xffff);
  int pr4=a4.p(pr, gc.c0^hash(gc.buf(1), gc.buf(2), gc.buf(3))&0xffff);
...
public:
  ...
  float num_bits;	// number of compressed bits
  ...
  int code(int i=0) {
	...
	num_bits+=8.0;
      ...
      x2=(x2<<8)+0xff;
      if (mode==DECOMPRESS) 
	x=(x<<8)+(getc(archive)&0xff);  // EOF is OK
    }
    return gc.y;
  }

  Encoder(GlobalContext& gc, Mode m, FILE* f);
...
Encoder::Encoder(GlobalContext& _gc, Mode m, FILE* f): 
  gc(_gc), predictor(gc), 
  mode(m), archive(f), x1(0), x2(0xffffffff), x(0), alt(0), num_bits(0) {
  if (gc.level>0 && mode==DECOMPRESS) {  // x = first 4 bytes of archive
    for (int i=0; i<4; ++i)
...
void Encoder::flush() {
  if (mode==COMPRESS && gc.level>0) {
    ...
    num_bits += bits;
    */
    float bits =  32.0 - log2( x2 + 1.0 - x1);
    //printf("num_bits=%d, zibar=%d\n", num_bits, bits);
    num_bits += bits ; // this ought to be faster
  }
}
...
// Compress a file
void compress(GlobalContext& gc, const char* filename, long filesize, Encoder& en) {
...
//        printf("%s already exists\n", archiveName.c_str()), quit();
    ...
    Encoder en(gc, mode, archive);
    if (mode==COMPRESS) {
      for (int i=0; i<files; ++i)
        compress(gc, fname[i], fsize[i], en);
      en.flush();
      printf("number of bits=%f\n", en.num_bits);
      printf("%ld -> %ld\n", total_size, en.size());
    }
...
void test_compress() {
  // Set globals according to option
  Buf buf;
  GlobalContext gc(buf);
  FILE *input = fopen("smalll.data", "r");
  FILE *archive = fopen("smalll.data.paq8ftest", "w");
  Encoder en(gc, COMPRESS, archive);
  int c;
  while (!feof(input)) {
    c = getc(input);
    en.compress(c);
...
static Buf shared_buf;

float compressed_size_bitstring(char *bitstring, int length) {
  GlobalContext gc(shared_buf);
  int byte_length = length/8;
  int last_bits = length % 8;
  if (last_bits > 0)
    byte_length++;
  FILE *input = fmemopen(bitstring, byte_length, "r");
  char *archive_str = new char[byte_length + 256];
  FILE *archive = fmemopen(archive_str, byte_length + 256, "w");
  Encoder en(gc, COMPRESS, archive);  
  char *curr_byte = bitstring;
  assert(en.getMode()==COMPRESS);
  for (int i=0; i<length / 8; i++) {
    //printf("compressing byte\n");
    en.compress(int(*curr_byte));
    curr_byte++;
  }
  for (int i=7; i>=8-last_bits; --i) {
    int bit = ((*curr_byte)>>i)&1;
    //printf("compressing bit=\n");
    en.code(bit);
  }
  en.flush();
  float num_bits = en.num_bits;
  //printf("#inputbits: %d, #compressedbits: %d\n", length, num_bits);
  fclose(input);
  fclose(archive);
  delete[] archive_str;
  return num_bits;
